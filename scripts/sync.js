            ...
            if (exists == false) {
              ...
              db.update_db(settings.coin, function(){
                db.get_stats(settings.coin, function(stats){
                  ...
                  if (mode == 'reindex') {
                  ...
                  } else if (mode == 'committx') { // manually add missing txs
                    txids = [
                    //  "4afaf607f96c04d1f4681525e7592f1f243d9169a76ea9798a5aa0f03541151e"
                    ];
                    explorer.syncLoop(txids.length, function(loop){
                      var i = loop.iteration();
                      console.log(txids[i]);
                      // TODO: that can add entries already in address.txs (and mess the balances)
                      db.reindex_one_tx(txids[i], function(err){ loop.next(); });
                    }, function(){
                      exit();
                    });
                  } else if (mode == 'check') {
                    var sinceHeight = 359214;
                    //db.purge_after(348000); exit();
                    db.update_tx_db(settings.coin, sinceHeight, stats.count, settings.check_timeout, function(){
                      db.get_stats(settings.coin, function(nstats){
                    ...
                    var sinceHeight = 410000;
                    db.get_last_block_missing(sinceHeight, function(h){
                      db.update_tx_db(settings.coin, h, stats.count, settings.update_timeout, function(){
                      db.update_richlist('received', function(){
                        db.update_richlist('balance', function(){
                          db.get_stats(settings.coin, function(nstats){
