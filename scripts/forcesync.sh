...
forcesync() {
  ...
  blockhash=`curl -s http://explorer.devcore.io/api/getblockhash?height=$blockcount`
  echo "| ଓ hash : $blockhash"
  curl -s http://explorer.devcore.io/block/$blockhash > /dev/null
  echo "╘═══════════════════════════════>>"
}
...
startingblockcount=350000
endingblockcount=350005
echo "Syncing..."
