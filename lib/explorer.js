...
var base_server = 'http://explorer.devcore.io' + "/";
//var base_server = 'http://localexplorer.devcore.io' + "/";

// Allows for large coin supply
BigNumber.config({ ERRORS: false });
var base_url = base_server + 'api/';
const NodeCache = require( "node-cache" );
const tx_cache = new NodeCache({ stdTTL: 1000, checkperiod: 1200 });
...
module.exports = {
  ...
  get_hashrate: function(cb) {
    ...
    } else if(settings.nethash == 'netstakeweight') {
      var uri = base_url + 'getstakinginfo'
      request({uri: uri, json: true}, function (error, response, body) {
        if (body.netstakeweight) {
          body.netstakeweight = body.netstakeweight / 100000000;
          return cb(body.netstakeweight.toFixed());
        } else {
          return cb('-');
        }
      });
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      } else if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata' && vout[i].scriptPubKey.hasOwnProperty("addresses")) {
