var mongoose = require('mongoose')
  ...
  , addrindex = require('./addressindex')
...
function find_address_updated(hash, cb) {
  Address.findOne({a_id: hash}, function(err, address) {
    if(address) {
      // override indexed balance with addressindex one
      if (settings.addressindex) {
        addrindex.get_addressbalance(hash, function(balances) {
          if (balances) {
            address.balance = balances.balance;
            if (typeof balances.staked != "undefined") {
              address.staked = balances.staked;
              address.sent = balances.spent;
              address.received = address.balance + address.sent - address.staked;
            } else {
              address.staked = -100000000; // N/A
              address.received = balances.received;
              address.sent = balances.received - address.balance;
            }
            address.updtime = +(Date.now());
          }
          return cb(address);
        });
      } else {
      ...
      if ( hash == 'coinbase' ) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          if (unique == true) {
            ...
            if (settings.addressindex) {
              // get exact balance counters
              find_address_updated(hash, function(indexed) {
                Address.update({a_id:hash}, {
                  txs: tx_array,
                  received: (indexed ? indexed.received : received),
                  sent:     (indexed ? indexed.sent : sent),
                  staked:   (indexed ? indexed.staked : address.staked),
                  balance:  (indexed ? indexed.balance : received - sent),
                  updtime:  (indexed ? indexed.updtime : address.updtime)
                }, function() {
                  return cb();
                });
              });
...
module.exports = {
  ...
  get_address: function(hash, cb) {
    find_address_updated(hash, function(address){
      return cb(address);
    });
  },
  ...
  get_last_txs: function(count, min, cb) {
    Tx.find({'total': {$gt: min}}).sort({timestamp: 'desc', blockindex: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_last_block_missing: function(sinceHeight, cb) {
    var height = sinceHeight;
    var heights = [];
    Tx.aggregate([
      { $match: { blockindex: { $gte: height} } },
      { $group: { _id: "$blockindex" } },
      { $sort: { "_id": -1 } },
    ]).sort({_id: 'desc'}).limit(100).exec(function(err, txs){
       if (err) {
          console.log(err);
          return cb(sinceHeight);
       }
       txs.forEach(function(tx) {
         heights[tx._id] = tx._id;
         if (heights[1 + tx._id] == undefined)
           height = tx._id;
       });
       return cb(height);
    });
  ...
  purge_after: function(sinceHeight) {
    Tx.deleteMany({ blockindex: { $gte: sinceHeight } }, function (err){
      console.log(err);
      return;
    });
  ...
  reindex_one_tx(txid, cb) {
    // not good, can add txs twice! to many addresses ?
    //save_tx(txid, cb);
  }
};
