  ...
  lib.get_difficulty(function(difficulty) {
    ...
    if (difficulty['proof-of-work']) {
            if (settings.index.difficulty == 'Hybrid') {
              difficultyHybrid = difficulty['proof-of-stake'];
  ...
  lib.get_masternodelist(function(list) {
    ...
    for (var key in list) {
      ...
      if (list.hasOwnProperty(key)) {
        ...
        if (settings.masternodes.list_format.txs  === 0)
          mnItem.txs  = key;
        else if (settings.masternodes.list_format.txs > -1)
          mnItem.txs = mnData[settings.masternodes.list_format.txs - 1];
